...
// 6-17
float nativeObjectRef::n_punch(ServerActiveObject *sao, ServerActiveObject *puncher,
	...
	if (dst_origin_hp != puncher->getHP() &&
		...
		getServer(L)->SendPlayerHPOrDie((PlayerSAO *)puncher,
				PlayerHPChangeReason(
						PlayerHPChangeReason::PLAYER_PUNCH, sao));
	}
...
void nativeObjectRef::n_set_hp(
		ServerActiveObject *sao, int hp, const PlayerHPChangeReason &reason)
{
	sao->setHP(hp, reason);
...
// 6-23
std::pair<bool, ItemGroupList> nativeObjectRef::n_set_armor_groups(
		ServerActiveObject *sao, const ItemGroupList &groups)
{
	if (sao == nullptr)
...
{
	// Assuming ServerActiveObject has a function getArmorGroups() that returns the
	// armor groups. Replace 'getArmorGroups()' with the actual function name if
	// different.
	return sao->getArmorGroups();
}

// 7-1
void nativeObjectRef
		: n_set_animation(ServerActiveObject *sao, v2f frame_range,
				  float frame_speed, float frame_blend, bool frame_loop)
{
	sao->setAnimation(frame_range, frame_speed, frame_blend, frame_loop);
}

void nativeObjectRef::n_get_animation(ServerActiveObject *sao, v2f *frames,
		float *frame_speed, float *frame_blend, bool *frame_loop)
{
	sao->getAnimation(frames, frame_speed, frame_blend, frame_loop);
}

void nativeObjectRef::n_set_local_animation(
		RemotePlayer *player, v2s32 frames[4], float frame_speed)
{
	getServer(L)->setLocalPlayerAnimations(player, frames, frame_speed);
}

std::pair<std::array<v2s32, 4>, float> nativeObjectRef::n_get_local_animation(
		RemotePlayer *player)
{
...
// Might not be changable
void nativeObjectRef::n_set_eye_offset(
		RemotePlayer *player, const v3f &offset_first, const v3f &offset_third)
{
...
void nativeObjectRef::n_set_animation_frame_speed(
		ServerActiveObject *sao, float frame_speed)
{
...
void nativeObjectRef::n_get_bone_position(ServerActiveObject *sao,
...
std::tuple<bool, std::string, v3f, v3f, bool> nativeObjectRef::n_set_attach(
		ServerActiveObject *sao, ServerActiveObject *parent,
		const std::string &bone, const v3f &position, const v3f &rotation,
		bool force_visible)
{
	...
	sao->getAttachment(&parent_id, &current_bone, &current_position,
			&current_rotation, &current_force_visible);
	if (parent_id) {
		ServerActiveObject *old_parent =
				sao->getEnv()->getActiveObject(parent_id);
	...
	return std::make_tuple(true, current_bone, current_position, current_rotation,
			current_force_visible);
}

// 7-9
int nativeObjectRef::n_get_attach(ServerActiveObject *sao, int *parent_id,
		std::string *bone, v3f *position, v3f *rotation, bool *force_visible)
{
...
// 7-14
void nativeObjectRef::n_set_properties(
		ServerActiveObject *sao, const ObjectProperties &prop)
{
	// No map lock required for this function, as it should be called from other C++
	// functions
	if (sao == nullptr)
		return;
...
std::pair<bool, ObjectProperties *> nativeObjectRef::n_get_properties(
...
bool nativeObjectRef::n_set_nametag_attributes(ServerActiveObject *sao,
		video::SColor nametag_color, video::SColor nametag_bgcolor,
		const std::string &nametag)
{
	...
	prop->nametag_bgcolor =
			(nametag_bgcolor.getAlpha() == 0) ? nullopt : nametag_bgcolor;
...
// 7-21
std::tuple<bool, Color, bool, Color, std::string>
nativeObjectRef::n_get_nametag_attributes(ServerActiveObject *sao)
{
	NO_MAP_LOCK_REQUIRED;
	if (sao == nullptr)
		return std::make_tuple(false, Color(255, 255, 255, 255), false,
				Color(0, 0, 0, 0), "");
...
void nativeObjectRef::n_set_attribute(ServerActiveObject *sao,
...
std::string nativeObjectRef::n_get_attribute(
		ServerActiveObject *sao, const std::string &attribute)
{
	PlayerSAO *playersao = dynamic_cast<PlayerSAO *>(sao);
	if (playersao) {
		std::string value;
...
void nativeObjectRef::n_set_fov(ServerActiveObject *sao, float degrees,
		bool is_multiplier, float transition_time)
{
...
v3f nativeObjectRef::n_get_look_dir(ServerActiveObject *sao)
{
	...
	return v3f(std::cos(pitch) * std::cos(yaw), std::sin(pitch),
...
std::string NativeObjectRef::get_entity_name()
{
	// Skipping the NO_MAP_LOCK_REQUIRED for simplicity
	LuaEntitySAO *entitysao = getluaobject(this);
	if (entitysao == nullptr) return "";

	return entitysao->getName();
}

LuaEntitySAO* NativeObjectRef::get_luaentity()
{
	LuaEntitySAO *entitysao = getluaobject(this);
	return entitysao;
}

std::string NativeObjectRef::get_player_name()
{
	RemotePlayer *player = getplayer(this);
...
int ObjectRef::native_override_day_night_ratio(float ratio) {
	NO_MAP_LOCK_REQUIRED;
	RemotePlayer *player = getplayer(this); // Assuming getplayer works with `this`
	if (player == nullptr)
		return 0; // Or any other error code you prefer

	bool do_override = (ratio >= 0.0f && ratio <= 1.0f);
	if(!do_override)
		return -1; // Or another error code

	getServer()->overrideDayNightRatio(player, do_override, ratio); // Assuming
getServer works without Lua state return 1; // Return success
}

float ObjectRef::native_get_day_night_ratio() {
	NO_MAP_LOCK_REQUIRED;
	RemotePlayer *player = getplayer(this); // Assuming getplayer works with `this`
	if (player == nullptr)
		return -1.0f; // Or any other error indicator
	...
	if(do_override)
		...
		return -1.0f; // Indicating no override is set
}
